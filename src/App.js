...
const procesarProductosWordPress = async (productosWP) => {
  ...
  if (!productosWP || productosWP.length === 0) {
    console.log('No products found');
    return [];
  }
  
  console.log('Processing products:', productosWP.length);
      ...
      for (const variacionId of producto.variations) {
        try {
          ...
          const response = await fetch(
            `${WP_URL}/wp-json/wc/v3/products/${producto.id}/variations/${variacionId}`,
            {
  ...
  console.log('Formatted products:', productosFormateados);
...
const App = () => {
  ...
  const cargarProductos = async () => {
    ...
    try {
      ...
      console.log('Raw WP response:', productosWP);
      
      if (!productosWP) {
        alert('No se pudo conectar a WordPress API. Verifica que el sitio sea accesible.');
        setLoading(false);
        return;
      }
      ...
      if (productosFormateados.length === 0) {
        alert('No se cargaron productos. Verifica que WordPress tenga productos con variantes.');
      }
    } catch (error) {
